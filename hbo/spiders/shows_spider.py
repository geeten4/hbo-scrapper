...
class ShowsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #list series je zatim prazdny, v pipelinu HboPipeline se do nej prubezne ulozi vsechny serie serialu
        yield GenericInfoItem(
            name_original = details.css("span.original-title::text").get(),
            year = meta[0],
            genre = meta[1],
            series=[]
        )
    ...
    def parse_series(self, response):
        ...
        #cislo serie
        series_number = response.css("div.season-tab a.selected::text").get()
        #list urls jednotlivych epizod
        episodes = response.css("div.shelf-modal-episode-item a::attr(href)").getall()
